...
# Task name pattern that indicates Polaris integration
...
if (.stages? // null) != null then
  ...
  # Case 2: steps-only pipeline -> convert to stages
  (.steps // []) as $steps
...
for f in "${files[@]}"; do
  ...
  # Robust detection: single boolean result
  DETECT_VAL="$(
    POLARIS_TASK_REGEX="$POLARIS_TASK_REGEX" "$YQ_BIN" e \
      'any(.. | select(tag=="!!map") | .task? // empty; test(strenv(POLARIS_TASK_REGEX)))' \
      "$f" 2>/dev/null || echo "false"
  )"
  echo "Detection eval: $DETECT_VAL"
 
  if [[ "$DETECT_VAL" != "true" ]]; then
